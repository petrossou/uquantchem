...
SUBROUTINE VLYPA(rha,rhb,grha,grhb,Vc)
             ...
             DOUBLE PRECISION :: omeg,delta,domeg,ddelta,ep1,ep2,dedrh,dedrha,dedrhb
             ...
             !==========================
             !ep = 1.0d0 + d*rh**(-1.0d0/3.0d0)
             !zi = (c + d/ep)*rh**(-1.0d0/3.0d0)
             !it = (1.0d0/3.0d0)*( (d/ep)**2*rh**(-1.0d0/3.0d0) - c - (d/ep) )*rh**(-4.0d0/3.0d0)
             !th = (ga*rha + gb*rhb)/rh
             !ka = (rha*rhb/18.0d0)*( 2*(11.0d0 - zi)*th + (47.0d0 - 7*zi)*gz + ( zi - 45.0d0)*(ga+gb) + 144*2.0d0**(2.0d0/3.0d0)*CF*(rha**(8.0d0/3.0d0) + rhb**(8.0d0/3.0d0) ) )
             !la = -2*rh**2*gz/3.0d0 + gb*( 2*rh**2/3.0d0 - rha**2 ) + ga*( 2*rh**2/3.0d0 - rhb**2 )

             !Vc(1) = (a/(3.0d0*ep**2))*rh**(-7.0d0/3.0d0)*(  -b*EXP(-c*rh**(-1.0d0/3.0d0))*( ka + la )*( (d + c*ep)*rh**(14.0d0/3.0d0) + 11.0d0*ep*rh**5 ) - 12.0d0*ep*rh**(4.0d0/3.0d0)*rhb &
             !& -4.0d0*d*rha*rhb + 12.0d0*ep*rh**(1.0d0/3.0d0)*rha*rhb - (b/6.0d0)*EXP(-c*rh**(-1.0d0/3.0d0))*ep*rh**6*( 24.0d0*rh*(ga-gz) + 12.0d0*gb*(2.0d0*rh-3.0d0*rha) + 18.0d0*ka/rha &
             !& + (rha*rhb/rh)*( 2.0d0*th*( zi - it*rh - 11.0d0) - 7.0d0*it*rh*gz + (22.0d0 - 2.0d0*zi + it*rh)*ga + it*rh*gb + 384.0d0*2.0d0**(2.0d0/3.0d0)*rh*CF*rha**(5.0d0/3.0d0) ) ) )
             !Vc(2) = (a/(3.0d0*ep**2))*rh**(-7.0d0/3.0d0)*(  -b*EXP(-c*rh**(-1.0d0/3.0d0))*( ka + la )*( (d + c*ep)*rh**(14.0d0/3.0d0) + 11.0d0*ep*rh**5 ) - 12.0d0*ep*rh**(4.0d0/3.0d0)*rha &
             !& -4.0d0*d*rha*rhb + 12.0d0*ep*rh**(1.0d0/3.0d0)*rha*rhb - (b/6.0d0)*EXP(-c*rh**(-1.0d0/3.0d0))*ep*rh**6*( 24.0d0*rh*(gb-gz) + 12.0d0*ga*(2.0d0*rh-3.0d0*rhb) + 18.0d0*ka/rhb &
             !& + (rha*rhb/rh)*( 2.0d0*th*( zi - it*rh - 11.0d0) - 7.0d0*it*rh*gz + (22.0d0 - 2.0d0*zi + it*rh)*gb + it*rh*ga + 384.0d0*2.0d0**(2.0d0/3.0d0)*rh*CF*rhb**(5.0d0/3.0d0) ) ) )
             
             ! DOUBLE PRECISION :: omeg,delta,domeg,ddelta,ep1,ep2,dedrh
             !================
             ! Derived by hand
             !================
             ...
             domeg = omeg*( (c/3.0d0)*rh**(-4.0d0/3.0d0) - 11.0d0/(3.0d0*rh) + (d/3.0d0)*rh**(-4.0d0/3.0d0)/(1.0d0 + d*rh**(-1.0d0/3.0d0) ) )
             ...
             dedrh = ( 4.0d0*rha*rhb*a/( (1.0d0 + d*rh**(-1.0d0/3.0d0))**2*rh**2 ) )*( 1.0d0 + (2.0d0/3.0d0)*d*rh**(-1.0d0/3.0d0) ) - a*b*domeg*(rha*rhb*ep1 + ep2) &
             & -a*b*omeg*( rha*rhb*(  (1.0d0/18.0d0)*ddelta*( ga + gb - 7.0d0*gz) + (1.0d0/9.0d0)*( (delta - 11.0d0)/rh - ddelta )*( (rha/rh)*ga + (rhb/rh)*gb ) )  &
             & + (4.0d0/3.0d0)*rh*( ga + gb - gz ) )

             dedrha = -a*b*omeg*( rhb*ep1 + rha*rhb*( 2.0d0**(11.0d0/3.0d0)*CF*(8.0d0/3.0d0)*rha**(5.0d0/3.0d0) - (1.0d0/9.0d0)*(delta - 11.0d0)*(ga/rh) ) -2.0d0*rha*gb ) - &
             & 4.0d0*rhb*a/(rh + d*rh**(2.0d0/3.0d0) )
             dedrhb = -a*b*omeg*( rha*ep1 + rha*rhb*( 2.0d0**(11.0d0/3.0d0)*CF*(8.0d0/3.0d0)*rhb**(5.0d0/3.0d0) - (1.0d0/9.0d0)*(delta - 11.0d0)*(gb/rh) ) -2.0d0*rhb*ga ) - &
             & 4.0d0*rha*a/(rh + d*rh**(2.0d0/3.0d0) )
...
!======================================================================================
        ...
        FUNCTION epsPC(rs)
                ...
                DOUBLE PRECISION :: rs
                DOUBLE PRECISION, PARAMETER :: A = 0.06218140d0
       ...
       END FUNCTION alphac
                ...
                FUNCTION muFC(rs)
                        ...
                        DOUBLE PRECISION, PARAMETER :: b = 7.060420d0
                        DOUBLE PRECISION, PARAMETER :: c = 18.05780d0
                        DOUBLE PRECISION, PARAMETER :: x0 = -0.325000d0
                        DOUBLE PRECISION :: b1,b2,b3
