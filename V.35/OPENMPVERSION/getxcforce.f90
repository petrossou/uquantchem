SUBROUTINE getxcforce(CORRLEVEL,NATOMS,ATOMS,NTOTALQUAD,Q1,Q2,Q3,BAS,Pup,Pdown,gradS,LORDER,CGORDER,LQ,CGQ,xcforce)
        ...
        DOUBLE PRECISION :: rm,rad,p,ptot,x,faktor,rcutt,tempxcforce(NATOMS,3,NTOTALQUAD)
        DOUBLE PRECISION, ALLOCATABLE  :: r(:),excforcedens(:,:),gradp(:,:),sumforce(:)
        ...
        !$OMP PARALLEL &
        !$OMP PRIVATE(J,JJ,n,m,I,x,rm,rad,r,faktor,p,excforcedens,gradp,sumforce)
        ALLOCATE(r(3),excforcedens(NATOMS,3),gradp(NATOMS,3),sumforce(3))
        !$OMP DO 
        DO J=1,NTOTALQUAD
                n = Q1(J)
                m = Q2(J)
                ...
                IF ( rad .LE. rcutt ) THEN  ! Cut off radius of 10*rm  is employed
                        ...
                        tempxcforce(:,:,J) = 4*PI*p*CGQ(n,2)*LQ(m,3)*faktor*excforcedens*rad**2 &
                        & -4*PI*CGQ(n,2)*LQ(m,3)*faktor*excdr(CORRLEVEL,BAS,Pup,Pdown,r)*gradp*rad**2
                        ! Using translational invariance to
                        ! calculate the oncite contribution to the force
                        sumforce = 0.0d0
                        DO JJ=1,NATOMS
                                !IF ( JJ .NE. I ) sumforce = sumforce - xcforce(JJ,:)
                                IF ( JJ .NE. I ) sumforce = sumforce - tempxcforce(JJ,:,J)
                        ENDDO
                        !xcforce(I,:) = sumforce
                        tempxcforce(I,:,J) = sumforce
                ENDIF
        ENDDO
        !$OMP END DO
        DEALLOCATE(r,excforcedens,gradp,sumforce)
        !$OMP END PARALLEL
