FUNCTION quadcheck(CORRLEVEL,NATOMS,ATOMS,NTOTALQUAD,Q1,Q2,Q3,BAS,Pup,Pdown,gradS,LORDER,CGORDER,LQ,CGQ)
        ! This function integrates the charge-density using the
        ! fuzzy teatraheadron division of space by Becke, and the 
        ! Chebychev-Gauss (2:nd order) Lebedev quadrature.
        ! This function is to be used in the end of a DFT calculation
        ! to check that the qaudrature mesh is dens enough to at least 
        ! when used to integrate the charge-density give the correct 
        ! number of electrons
        USE datatypemodule
        IMPLICIT NONE
        ...
        DOUBLE PRECISION, ALLOCATABLE :: r(:),exxcc(:)
        ...
        DO J=1,NTOTALQUAD
                ...
                IF ( rad .LE. rcutt ) THEN 
                        ...
                        quadcheck = quadcheck + 4*PI*p*CGQ(n,2)*LQ(m,3)*faktor*rho(BAS,Pup+Pdown,r)*rad**2
                ENDIF
        ENDDO
