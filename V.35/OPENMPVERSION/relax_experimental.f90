SUBROUTINE relax(gradS,gradT,gradV,gradIntsv,S,H0,Intsv,NB,NRED,Ne,nucE,Tol,FTol,MIX,DIISORD,DIISSTART,NATOMS,NSTEPS,DR,BAS,ATOMS,APPROXEE,CORRLEVEL,PRYSR,PRYSW)
        ...
        DOUBLE PRECISION :: leng,lengold, f(3), g(3),Rn(3),force(NATOMS,3),T(NB,NB),V(NB,NB),R0(NATOMS,3),R1(NATOMS,3),R2(NATOMS,3)
        DOUBLE PRECISION :: forceold(NATOMS,3),gradold(NATOMS,3),grad(NATOMS,3),E0,E1,E2,E3,E4,const1,const2,DRM,DRMO,Ebis,Eprim,Etris,Equar,nom,FACTOR
      ...
      !=======================================================================================================================
       ...
       !=========================================================================================================================================
                ...
                DO WHILE ( LSEARCH )
                        IF ( DABS(Eprim) .LT. FTol .OR. KK .EQ. NLITER ) LSEARCH = .FALSE.
                        ...
                        DO J=JSTART,JEND
                                ...
                                IF ( J .EQ. 5 ) THEN 
                                    ...
                                    Equar = (-E3 - 4*E1 + 6*E0 - 4*E2 + E4)/(DR**4)

                                    ! Using Taylor expansion up to 2:nd order to find minimum
                                    IF ( Eprim .NE. 0.0d0 .AND. Ebis .NE. 0.0d0 ) THEN
                                            DRM = DRM - Eprim/dabs(Ebis)
                                    ENDIF

                                    ! Using Taylor expansion up to 3:rd order to  find minimum
                                    ...
                                    ENDIF
                                    ! Safety cutoff:
                                    IF ( DABS(DRM) .GT.  10.0d0 ) DRM = 10.0d0*DRM/DABS(DRM)
                                ENDIF
       ...
       !=======================================================================================================================
                ...
                WRITE(*,'(I4,E30.20,E30.20,E30.20,E30.20,I4)')I,E0,DE,leng,DRM,KK
