SUBROUTINE makedensTc(TOLDNe,Cup,Cdown,Eu,Ed,ETEMP,NB,Ne,Pup,Pdown,mu,ENTROPY)
      ...
      COMPLEX*16, INTENT(INOUT) :: Cup(NB,NB),Cdown(NB,NB)
      DOUBLE PRECISION, INTENT(IN) ::  Eu(NB),Ed(NB),ETEMP
      DOUBLE PRECISION, INTENT(OUT) :: ENTROPY
      COMPLEX*16, INTENT(OUT) :: Pup(NB,NB),Pdown(NB,NB)
      DOUBLE PRECISION :: OCC,DOCC,OCCsave,DDOCC
      COMPLEX*16 :: Pups(NB,NB),Pdowns(NB,NB),COEFFus(NB,NB),COEFFds(NB,NB)
      DOUBLE PRECISION :: SHI(NB,NB),DNel,DNels,muo,fu,fd,musave
      COMPLEX*16 :: COEFFu(NB,NB),COEFFd(NB,NB),ID(NB,NB),PT(NB,NB)
