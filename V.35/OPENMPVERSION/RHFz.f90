SUBROUTINE RHFz(S,H0,Intsv,NB,NRED,Ne,nucE,Tol,EHFeigen,ETOT,C,P,MIX,DIISORD,DIISSTART,NSCF,FIXNSCF,POUT,SCRATCH,ZEROSCF,Jup,Jdown,Kup,Kdown,NBAUX,VRI,WRI,RIAPPROX)
      ...
      DOUBLE PRECISION, INTENT(IN) :: S(NB,NB),H0(NB,NB),Intsv(NRED),Tol,nucE,MIX,VRI(NBAUX,NBAUX),WRI(NB,NB,NBAUX)
      ...
      !=======================================================
      IF ( ZEROSCF ) THEN
                CALL getJv(P,NB,NBAUX,NRED,Intsv,VRI,WRI,RIAPPROX,J)
                CALL getKv(P,NB,NBAUX,NRED,Intsv,VRI,WRI,RIAPPROX,K)
                F = H0 + J - 0.50d0*K
                G = J - 0.50d0*K
      ...
      DO WHILE ( (DABS(DE) .GT. Tol .AND. I .LE. MAXITER) .OR. ( DELTAP .GT. sqrt(Tol) .AND. I .LE. MAXITER) )
                ...
                IF ( FIXNSCF .LT. 0 ) ETOT = SUM(F*P)-0.50d0*SUM(G*P) + nucE
