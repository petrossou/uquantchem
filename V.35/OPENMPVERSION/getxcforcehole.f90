SUBROUTINE getxcforcehole(CORRLEVEL,NATOMS,ATOMS,NTOTALQUAD,Q1,Q2,Q3,BAS,Pup,Pdown,gradS,LORDER,CGORDER,LQ,CGQ,xcforce,IORBNR,PHOLE)
        ...
        INTEGER, INTENT(IN) :: NATOMS,LORDER,CGORDER,NTOTALQUAD,Q1(NTOTALQUAD),Q2(NTOTALQUAD),Q3(NTOTALQUAD),IORBNR
        ...
        DOUBLE PRECISION, INTENT(IN) :: LQ(LORDER,3),CGQ(CGORDER,2),PHOLE(BAS%NBAS,BAS%NBAS)
        ...
        DO J=1,NTOTALQUAD
                ...
                IF ( rad .LE. rcutt ) THEN  ! Cut off radius of 10*rm  is employed
                        ...
                        CALL dftforcedenshole(CORRLEVEL,NATOMS,BAS,Pup,Pdown,gradS,r,I,excforcedens,IORBNR,PHOLE)
                        ...
                        ELSE
                                IF ( IORBNR .GT. 0 ) THEN
                                        xcforce = xcforce -4*PI*CGQ(n,2)*LQ(m,3)*faktor*excdr(CORRLEVEL,BAS,Pup-PHOLE,Pdown,r)*gradp*rad**2
                                ELSE
                                        xcforce = xcforce -4*PI*CGQ(n,2)*LQ(m,3)*faktor*excdr(CORRLEVEL,BAS,Pup,Pdown-PHOLE,r)*gradp*rad**2
                                ENDIF
                        ENDIF
