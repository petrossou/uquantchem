SUBROUTINE forces(CORRLEVEL,NATOMS,Ne,NB,NRED,Cup,Cdown,Pup,Pdown,EHFeigenup,EHFeigendown,ATOMS,BAS,gradS,gradT,gradV,gradIntsv,S,H0,Intsv,PULAY,NTOTALQUAD,Q1,Q2,Q3,LORDER,&
                 & CGORDER,LQ,CGQ,force,EDIR,EPROFILE,EFIELD,OMEGA,TIME,ADEF,NBAUX,VRI,WRI,gradVRI,gradWRI,RIAPPROX)
      USE datatypemodule
      IMPLICIT NONE
      ...
      DOUBLE PRECISION, INTENT(IN) :: LQ(LORDER,3),CGQ(CGORDER,2),EFIELD,OMEGA,TIME,VRI(NBAUX,NBAUX),WRI(NB,NB,NBAUX)
      ...
      DOUBLE PRECISION :: PT(NB,NB),Jup(NB,NB),Jdown(NB,NB),Kup(NB,NB),Kdown(NB,NB),Cu(NB,NB),Cd(NB,NB),gDPTENSOR(NATOMS,3,NB,NB),DPTENSOR(NB,NB),DPTENSORT(3,NB,NB)
      DOUBLE PRECISION :: Jupgrad(NB,NB),Jdowngrad(NB,NB),Kupgrad(NB,NB),Kdowngrad(NB,NB),xcforce(NATOMS,3),dPdR(NB,NB),sumforce(3),gDPTENSORC(NATOMS,3,NB,NB)
      DOUBLE PRECISION, INTENT(OUT) :: force(NATOMS,3)
      DOUBLE PRECISION :: Fup(NB,NB),Fdown(NB,NB),SI(NB,NB),Vxc(2,NB,NB),cl,INVRI(NBAUX,NBAUX)
      INTEGER, EXTERNAL :: ijkl
      DOUBLE PRECISION, EXTERNAL :: TRACE
      INTEGER :: Neup, Nedown,I,J,K,N,II,JJ,KK,LL
      ...
      IF ( ADEF ) THEN
              ...
              ELSE
                      ...
                      IF ( EPROFILE .EQ. 'CIRC' ) THEN
                              IF ( EDIR .EQ. 1 ) CALL graddipoletensor(NATOMS,BAS,3,gDPTENSORC)
                              IF ( EDIR .EQ. 2 ) CALL graddipoletensor(NATOMS,BAS,1,gDPTENSORC)
                              IF ( EDIR .EQ. 3 ) CALL graddipoletensor(NATOMS,BAS,2,gDPTENSORC)
                      ENDIF
              ENDIF
      ENDIF   
      Cu(:,:) = 0.0d0
      ...
      IF ( CORRLEVEL .EQ. 'B3LYP' .OR. CORRLEVEL .EQ. 'RHF' .OR. CORRLEVEL .EQ.  'URHF' ) THEN
      ...
      DO N=1,NATOMS
                DO J=1,3
                        IF ( .not. RIAPPROX ) THEN
                                CALL getJv(Pup,NB,NBAUX,NRED,gradIntsv(N,J,:),VRI,WRI,RIAPPROX,Jupgrad)
                                CALL getJv(Pdown,NB,NBAUX,NRED,gradIntsv(N,J,:),VRI,WRI,RIAPPROX,Jdowngrad)
                                IF ( CORRLEVEL .EQ. 'B3LYP' .OR. CORRLEVEL .EQ.  'RHF' .OR. CORRLEVEL .EQ.  'URHF' ) THEN
                                        CALL getKv(Pup,NB,NBAUX,NRED,gradIntsv(N,J,:),VRI,WRI,RIAPPROX,Kupgrad)
                                        CALL getKv(Pdown,NB,NBAUX,NRED,gradIntsv(N,J,:),VRI,WRI,RIAPPROX,Kdowngrad)
                                ENDIF
                        ELSE
                                CALL getJvforceRI(Pup,NB,NBAUX,NATOMS,J,N,VRI,WRI,gradVRI,gradWRI,Jupgrad)
                                CALL getJvforceRI(Pdown,NB,NBAUX,NATOMS,J,N,VRI,WRI,gradVRI,gradWRI,Jdowngrad)
                                IF ( CORRLEVEL .EQ. 'B3LYP' .OR. CORRLEVEL .EQ.  'RHF' .OR. CORRLEVEL .EQ.  'URHF' ) THEN
                                        CALL getKvforceRI(Pup,NB,NBAUX,NATOMS,J,N,VRI,WRI,gradVRI,gradWRI,Kupgrad)
                                        CALL getKvforceRI(Pdown,NB,NBAUX,NATOMS,J,N,VRI,WRI,gradVRI,gradWRI,Kdowngrad)
                                ENDIF
                        ENDIF
                        IF ( CORRLEVEL .EQ. 'RHF' .OR. CORRLEVEL .EQ. 'URHF' ) THEN
                                ...
                                IF ( ADEF ) THEN
                                        ...
                                        ELSE
                                                force(N,J) = force(N,J) - EFIELD*( SUM(gDPTENSOR(N,J,:,:)*PT)*sin(OMEGA*TIME) + SUM(gDPTENSORC(N,J,:,:)*PT)*cos(OMEGA*TIME))
                                        ENDIF
                                ENDIF
                        ...
                        ELSE IF (  CORRLEVEL .EQ. 'PBE' .OR. CORRLEVEL .EQ.  'LDA' .OR. CORRLEVEL .EQ. 'B3LYP' ) THEN
                                ...
                                IF ( N .EQ.1 .AND. J .EQ. 1  ) THEN
                                        ...
                                        ELSE
                                                CALL getxcforcenip(CORRLEVEL,NATOMS,ATOMS,NTOTALQUAD,Q1,Q2,Q3,BAS,Pup,Pdown,LORDER,CGORDER,LQ,CGQ,xcforce)
                ...
                IF ( ADEF ) THEN
                        IF ( EPROFILE .NE. 'AC' ) THEN
                                ...
                                ELSE
                                        IF ( EDIR .EQ. 1 ) THEN
                                                force(N,2) = force(N,2) + EFIELD*sin(OMEGA*TIME)*ATOMS(N)%Z
                                                force(N,3) = force(N,3) + EFIELD*cos(OMEGA*TIME)*ATOMS(N)%Z
                                        ELSE IF ( EDIR .EQ. 2 ) THEN
                                                force(N,3) = force(N,3) + EFIELD*sin(OMEGA*TIME)*ATOMS(N)%Z
                                                force(N,1) = force(N,1) + EFIELD*cos(OMEGA*TIME)*ATOMS(N)%Z
                                        ELSE IF ( EDIR .EQ. 3 ) THEN
                                                force(N,1) = force(N,1) + EFIELD*sin(OMEGA*TIME)*ATOMS(N)%Z
                                                force(N,2) = force(N,2) + EFIELD*cos(OMEGA*TIME)*ATOMS(N)%Z
                                        ENDIF
                                ENDIF
                        ELSE
