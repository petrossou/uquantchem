SUBROUTINE readbasisaux(NATOMS,ATOMS,BASISMAP,MAP)
      ...
      1 FORMAT(A200)
      IF ( MAP ) THEN
              OPEN(21,FILE='BASISFILEAUX',STATUS='OLD',ACTION='READ')
              NLINES = 0
                ...
                DO K=1,NATOMS
                        ...
                        DO WHILE (.TRUE.)
                                ...
                                ELSE
                                        ...
                                        DO I=1,BASISMAP(Z)
                                                        DO J=1,PRIMS(I)
                                                        IF ( STORE ) READ(21,*,end=2000)LMOM,ATOMS(K)%PSI(I)%EXPON(J),ATOMS(K)%PSI(I)%CONTRCOEFF(J)
                                                                IF ( .not. STORE)READ(21,FMT=1,end=2000)LINE
                                                        ENDDO
                                                        IF ( STORE ) THEN
                                                                ATOMS(K)%PSI(I)%L(:) = 0
