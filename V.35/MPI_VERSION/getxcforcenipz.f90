SUBROUTINE getxcforcenipz(CORRLEVEL,NATOMS,ATOMS,BAS,Pup,PNup,Pdown,PNdown,LORDER,CGORDER,LQ,CGQ,NTOTALQUAD,Q1,Q2,Q3,Qstart,Qend,numprocessors,id,xcforce)
        ...
        !=======================================================================
        ! The only difference between this routine and getxcforcenip.f90 is that 
        ! in this routine all nuclear gradients are calculated using the density 
        ! matrices 2*D - P, instead of just D as in getxcforcenip.f90. 
        !   D  <--> Pup,Pdown  and  P <--> PNup,PNdown
        !=======================================================================
        USE datatypemodule
        IMPLICIT NONE
        INCLUDE "mpif.h"
        ...
        DOUBLE PRECISION, INTENT(IN) :: Pup(BAS%NBAS,BAS%NBAS), Pdown(BAS%NBAS,BAS%NBAS),PNup(BAS%NBAS,BAS%NBAS), PNdown(BAS%NBAS,BAS%NBAS)
        ...
        DO J=Qstart,Qend
               ...
               IF ( rad .LE. rcutt ) THEN  ! Cut off radius of 10*rm  is employed
                                        ...
                                        CALL dftforcedensnipz(CORRLEVEL,NATOMS,BAS,Pup,PNup,Pdown,PNdown,r,I,excforcedens)
