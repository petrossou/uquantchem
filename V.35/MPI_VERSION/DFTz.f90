SUBROUTINE DFTz(CORRLEVEL,NATOMS,ATOMS,BAS,S,H0,Intsv,IND1,IND2,IND3,IND4,Istart,Iend,Q1,Q2,Q3,Qstart,Qend,NTOTALQUAD,NB,NRED,Ne,LORDER,CGORDER,LQ,CGQ,nucE,Tol,EHFeigenup,EHFeigendown, &
           & ETOT,Cup,Cdown,Pup,Pdown,MIX,DIISORD,DIISSTART,NSCF,FIXNSCF,numprocessors,id,POUT,SCRATCH,ZEROSCF,Jup,Jdown,Kup,Kdown,ETEMP,mu,ENTROPY,Vxc)
      ...
      DOUBLE PRECISION, INTENT(IN) ::  S(NB,NB),H0(NB,NB),Intsv(Istart:Iend),Tol,nucE,MIX,LQ(LORDER,3),CGQ(CGORDER,2),ETEMP
      DOUBLE PRECISION, INTENT(OUT) :: EHFeigenup(NB),EHFeigendown(NB),ETOT,mu,ENTROPY
      DOUBLE PRECISION, INTENT(INOUT) :: Cup(NB,NB),Cdown(NB,NB),Pup(NB,NB),Pdown(NB,NB)
      DOUBLE PRECISION, INTENT(OUT) :: Jup(NB,NB),Jdown(NB,NB),Kup(NB,NB),Kdown(NB,NB),Vxc(2,NB,NB)
      DOUBLE PRECISION :: PT(NB,NB),C3(NB,NB),C4(NB,NB)
...
10 CONTINUE
                ...
                IF ( id .EQ. 0 .AND. POUT ) THEN 
                   IF ( STARTPRINTDIISIFO ) THEN
                        WRITE(*,'(I4,E30.20,E30.20,E30.20,E30.20)'),I,ETOT,DE,DELTAP,LAMDAd+LAMDAu
                   ELSE
     ...
     IF ( I .LE. MAXITER  .OR. FIXNSCF .GT. 0  ) THEN
           ...
           IF ( id .EQ. 0 .AND. POUT ) THEN
             ...
             IF ( CORRLEVEL .NE. 'B3LYP' ) THEN
                        IF ( ETOT .GT. -1.0E03) WRITE(*,'(A6,A3,A15,E27.20,A3)'),'      ',CORRLEVEL,' energy:   E = ',ETOT,' au'
                        IF ( ETOT .LT. -1.0E03) WRITE(*,'(A6,A3,A15,E30.20,A3)'),'      ',CORRLEVEL,' energy:   E = ',ETOT,' au'
             ELSE
                        IF ( ETOT .GT. -1.0E03) WRITE(*,'(A6,A5,A15,E27.20,A3)'),'      ',CORRLEVEL,' energy:   E = ',ETOT,' au'
                        IF ( ETOT .LT. -1.0E03) WRITE(*,'(A6,A5,A15,E30.20,A3)'),'      ',CORRLEVEL,' energy:   E = ',ETOT,' au'
             ENDIF 
             print*,' '
