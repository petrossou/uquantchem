             ...
             20 CONTINUE    
                ...
                ELSE
                      ...
                      IF ( TOTALVIKT .GT. 0.0d0 .AND. DABS(CORRECTION/ER(I-1)) .LT. CUTTOFFFACTOR ) THEN
                          ...
                          ! due to the corruption of the array ER(:) after a restart. However I cannot 
                          ! find where it all goes wrong.
                          ! Instead we have to do the following ( see below )
                          IF ( RESTART ) ER(I) = ERMEAN - CORRECTION
                      ELSE
                          ER(I)  = ER(I-1)
                      ENDIF 
                ENDIF
