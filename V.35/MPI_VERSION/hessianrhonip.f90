SUBROUTINE hessianrhonip(NATOMS,BAS,P,r,BECKECENTER,hr)
      ...
      ! coordinate r.
      !=============================================================
      ! This subroutine is allmost the same as hessianrho.f90 except 
      ! that all contributions to the hessian emenating from nuclear 
      ! gradients applied to the density matrices have been excluded.
      !=============================================================
      USE datatypemodule
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: NATOMS,BECKECENTER
      TYPE(BASIS), INTENT(IN) :: BAS
      DOUBLE PRECISION, INTENT(IN) :: P(BAS%NBAS,BAS%NBAS),r(3)
      ...
      DO I=1,NATOMS
         ...
         hr(1,1,I) = 2*(  - DOT_PRODUCT(dPHImxx,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImx,MATMUL(P,dPHInx)) )
         hr(2,1,I) = 2*(  - DOT_PRODUCT(dPHImyx,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImx,MATMUL(P,dPHIny)) )
         hr(3,1,I) = 2*(  - DOT_PRODUCT(dPHImzx,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImx,MATMUL(P,dPHInz)) )

         hr(1,2,I) = 2*(  - DOT_PRODUCT(dPHImyx,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImy,MATMUL(P,dPHInx)) )
         hr(2,2,I) = 2*(  - DOT_PRODUCT(dPHImyy,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImy,MATMUL(P,dPHIny)) )
         hr(3,2,I) = 2*(  - DOT_PRODUCT(dPHImzy,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImy,MATMUL(P,dPHInz)) )

         hr(1,3,I) = 2*(  - DOT_PRODUCT(dPHImzx,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImz,MATMUL(P,dPHInx)) )
         hr(2,3,I) = 2*(  - DOT_PRODUCT(dPHImzy,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImz,MATMUL(P,dPHIny)) )
         hr(3,3,I) = 2*(  - DOT_PRODUCT(dPHImzz,MATMUL(P,PHIn)) - DOT_PRODUCT(dPHImz,MATMUL(P,dPHInz)) )
