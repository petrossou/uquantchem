SUBROUTINE forces(IND1,IND2,IND3,IND4,Istart,Iend,NATOMS,Ne,NB,NRED,Cup,Cdown,Pup,Pdown,EHFeigenup,EHFeigendown,ATOMS,BAS,gradS,gradT,gradV,gradIntsvR,S,H0,Intsv,PULAY,force,numprocessors,id, &
                 & Q1,Q2,Q3,Qstart,Qend,NTOTALQUAD,LORDER,CGORDER,LQ,CGQ,CORRLEVEL,EDIR,EPROFILE,EFIELD,OMEGA,TIME,ADEF)
      USE datatypemodule
      IMPLICIT NONE
      INCLUDE "mpif.h"
      ...
      DOUBLE PRECISION, INTENT(IN) :: LQ(LORDER,3),CGQ(CGORDER,2),EFIELD,OMEGA,TIME
      ...
      DOUBLE PRECISION :: Jup(NB,NB),Jdown(NB,NB),Kup(NB,NB),Kdown(NB,NB),Fup(NB,NB),Fdown(NB,NB),SI(Nb,NB),xcforce(NATOMS,3),sumforce(3),Vxc(2,NB,NB),cl,gDPTENSORC(NATOMS,3,NB,NB)
      ...
      IF ( CORRLEVEL .EQ. 'B3LYP' .OR. CORRLEVEL .EQ. 'URHF' .OR. CORRLEVEL .EQ.  'RHF' ) THEN
                        ...
                        ELSE IF (  CORRLEVEL .EQ. 'PBE' .OR. CORRLEVEL .EQ.  'LDA' .OR. CORRLEVEL .EQ. 'B3LYP' ) THEN
                                ...
                                IF ( N .EQ.1 .AND. J .EQ. 1  ) THEN
                                        IF ( PULAY .EQ. 4 ) THEN
                                                ...
                                                Fdown = H0 + Jdown + Jup
                                        ELSE
                                                CALL getxcforcenip(CORRLEVEL,NATOMS,ATOMS,BAS,Pup,Pdown,LORDER,CGORDER,LQ,CGQ,NTOTALQUAD,Q1,Q2,Q3,Qstart,Qend,numprocessors,id,xcforce)
