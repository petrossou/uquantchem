...
&               NRECALC,NOREDIST,REDISTRIBUTIONFREQ,CUTTOFFFACTOR,numprocessors,id)
        ...
        INTEGER, ALLOCATABLE :: NREJECTMERGED(:)
        DOUBLE PRECISION, ALLOCATABLE :: ER(:),E(:),force(:),xt(:),xtrial(:),xtemp(:,:),xold(:,:),xkilledc(:,:),EKILLEDC(:),WKILLEDC(:)
        ...
        DOUBLE PRECISION, ALLOCATABLE :: VBIG(:), VBIGMEAN(:),VBIGP(:), VBIGMEANP(:),WEIGHT(:),NEWWEIGHT(:),xkilled(:,:),WTOTC(:),EKILLED(:),WKILLED(:)
        ...
        ALLOCATE(xt(N3),xtrial(N3),xstart(N3,NREP2),offset(numprocessors),NKILLWC(numprocessors),NREJECTMERGED(NREP2))
        ALLOCATE(xtemp(N3,NREP2),NREJECT(NREP2),NREJECTNEW(NREP2),NREJECTUPPDATE(NREP2),xkilled(N3,NREP2),xkilledc(N3,NREP2),xmerged(N3,NREP2))
        ALLOCATE(force(N3),x1(N3),x2(N3),x3(N3),x4(N3),WEIGHT(NREP2),NEWWEIGHT(NREP2),EKILLEDC(NREP2),WKILLEDC(NREP2))
        ...
        DO I=2,NTIMESTEPS
                IF ( id .EQ. 0 ) WRITE(12,'(I20,F30.20,F30.20,F30.20,I20)')I,ET+nucE,ER(I-1)+nucE,E(I-1)+nucE,NWALKTOT
            10  CONTINUE
                COUN = 0
                WTOT = 0.0d0
                NACC = 0 
                ...
                DO J=1,NWALK(id+1)
                    ...
                    20  CONTINUE
                        ...
                        IF ( NUMBEROFNEWWALKERS .GE. 1  ) THEN
                                ...
                                ENDDO
                        ELSE
                                ! Here rejected walkers are saved in order to be merged in pairs after the loop over walkers
                ...
                DO WHILE ( K .LE. NWALKTOTKILLED )
                   ...
                   IF ( K+1 .LE. NWALKTOTKILLED ) THEN
                        ...
                        ENDIF
                   ELSE ! We put the walkers weights and number of killed walkers that haven't been merged in thread id=0
                             IF ( id .EQ. 0 ) THEN
                                NKILLW = 1
                                xkilled(:,NKILLW) = xkilledc(:,K)
                                WKILLED(NKILLW) = WKILLEDC(K)
                                EKILLED(NKILLW) = EKILLEDC(K)
                                NREJECTKILLED(NKILLW) = NREJECTKILLEDC(K)
                             ENDIF
                    ENDIF
                    K= K+2
                ENDDO
                ...
                ! The number of merged replicas/processor:
                IF ( NMERGED .NE. 0 ) THEN
                  ...
                  ENDDO
                  NWALKNEW(id+1) = NWALKNEW(id+1) + NWALK(id+1)
                ENDIF
                !----------------------------------------
                ...
                IF ( (ET-E(I))/abs(ET) .LT. CUTTOFFFACTOR ) THEN
                        ...
                        NWALK(id+1) = NWALKNEW(id+1)
                        COUNC(id+1) = NWALKNEW(id+1)
                        WEIGHT = NEWWEIGHT
                ELSE
                        print*,'PLACE6'
                        GOTO 10
                ...
                IF ( ( MINVAL(COUNC,1) .EQ. 0 .OR. MAXVAL(COUNC,1) .GE. 2*NRT .OR. DISTR ) .AND. .not. NOREDIST ) THEN
                   ...
                   CALL MPI_GATHERV(WEIGHT(1:COUNC(id+1)),rcountsreject(id+1),MPI_DOUBLE_PRECISION,NEWWEIGHT,rcountsreject,displsreject,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
                   ...
                   CALL MPI_GATHERV(NREJECT(1:COUNC(id+1)),rcountsreject(id+1),MPI_INTEGER,NREJECTNEW,rcountsreject,displsreject,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
                   ...
                   CALL MPI_BCAST(NEWWEIGHT,NREP2, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)

                   ! Sending the array of walker-rejection numbers, NREJECTNEW, located at thread 0 (id = 0 ) to all other threads.
                   CALL MPI_BCAST(NREJECTNEW,NREP2, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
                   ...
                   DO K=1,NWALK(id+1)
                       ...
                       NREJECT(K) = NREJECTNEW(offset(id+1)+K)
                       WEIGHT(K) =  NEWWEIGHT(offset(id+1)+K)
                   ENDDO
               ELSE
                  !----------------------------------------------------------------
