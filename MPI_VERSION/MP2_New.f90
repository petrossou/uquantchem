SUBROUTINE MP2(Cup,Cdown,Ints,NB,Ne,E0,nuce,EMP2,EHFeigenup,EHFeigendown)
      ...
      K1 = 1
       ...
       DO I=1,Ne
        ...
        !$OMP & PRIVATE(M,N,K,L,J,A,B,Q,DIFFER,V1,Ca,Calpha,Cb,Cbeta,TEMP1,DEIJKL)
            ...
            IF ( DIFFER .EQ. 2 ) THEN
                     ...
                     IF ( ( A(1) .LE. NB .AND. B(1) .LE. NB ) .OR. ( A(1) .GT. NB .AND. B(1) .GT. NB ) ) THEN
                             IF ( ( A(2) .LE. NB .AND. B(2) .LE. NB ) .OR. ( A(2) .GT. NB .AND. B(2) .GT. NB ) ) THEN
                                     ...
                                     DO M=1,NB
                                                DO K=1,NB
                                                TEMP1 = TEMP1 + Calpha(M)*Cbeta(K)*DOT_PRODUCT(Ca,MATMUL(Ints(M,K,:,:),Cb))
                                                ENDDO
                                     ENDDO
                               ENDIF
                      ENDIF
                      ...
                      IF ( ( A(1) .LE. NB .AND. B(2) .LE. NB ) .OR. ( A(1) .GT. NB .AND. B(2) .GT. NB ) ) THEN
                             IF ( ( A(2) .LE. NB .AND. B(1) .LE. NB ) .OR. ( A(2) .GT. NB .AND. B(1) .GT. NB ) ) THEN
                                     ...
                                     DO M=1,NB
                                                        DO L=1,NB
                                                        TEMP1 = TEMP1 - Calpha(M)*Cbeta(L)*DOT_PRODUCT(Ca,MATMUL(Ints(M,L,:,:),Cb))
                                                        ENDDO
                                     ENDDO
                              ENDIF
                      ENDIF 
