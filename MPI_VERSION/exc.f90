FUNCTION exc(CORRLEVEL,NATOMS,ATOMS,BAS,Pup,Pdown,LORDER,CGORDER,LQ,CGQ,NTOTALQUAD,Q1,Q2,Q3,Qstart,Qend,numprocessors,id)
        ...
        DOUBLE PRECISION :: excthread,erecieve(numprocessors)
        ...
        DO J=Qstart,Qend
                                ...
                                IF ( rad .LE. rcutt ) THEN 
                                        ...
                                        excthread = excthread + 4*PI*p*CGQ(n,2)*LQ(m,3)*faktor*excdr(CORRLEVEL,BAS,Pup,Pdown,r)*rad**2
                                ENDIF
        ENDDO
        CALL MPI_REDUCE(excthread,exc,1,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD, ierr)
        CALL MPI_BCAST(exc,1,MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
